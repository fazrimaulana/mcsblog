...
<div class="header-title">
            <h3>
                <i class="fa fa-thumb-tack" aria-hidden="true"></i> Page
                <a href="{{ url('/dashboard/pages/add') }}" id="new-post"><i class="fa fa-plus" aria-hidden="true"></i> Add New</a>
            </h3>
            ...
            <div class="content-post">
                ...
                <div class="menu-post col-md-12 col-sm-12 col-xs-12">
                    <div class="col-md-8 col-sm-8 col-xs-7 pull-left">
                        <ul class="nav nav-pills">
                            <li role="presentation" class=" {{ strchr(Route::currentRouteName(), 'pages') === 'pages' ? 'active' : '' }} " ><a href="{{ url('/dashboard/pages') }}">All <span class="badge">{{ $postCount->count() }}</span></a></li>
                            <li role="presentation" class=" {{ strchr(Route::currentRouteName(), 'published') === 'published' ? 'active' : '' }} " ><a href="{{ url('/dashboard/pages/published') }}">Published <span class="badge">{{ $postCount->where('status', 'published')->count() }}</span></a></li>
                            <li role="presentation" class=" {{ strchr(Route::currentRouteName(), 'draft') === 'draft' ? 'active' : '' }} " ><a href="{{ url('/dashboard/pages/draft') }}">Draft <span class="badge">{{ $postCount->where('status', 'draft')->count() }}</span></a></li>
                            <li role="presentation" class=" {{ strchr(Route::currentRouteName(), 'trash') === 'trash' ? 'active' : '' }} " ><a href="{{ url('/dashboard/pages/trash') }}">Trash <span class="badge">{{ $postCount->where('status', 'trash')->count() }}</span></a></li>
                        </ul>
                    </div>
                    <div class="col-md-3 col-sm-3 col-xs-4 pull-right">
                        <form action="{{ url('/dashboard/pages/search') }}" method="get">
                ...
                <div class="filter-post col-md-12 col-sm-12">
                    ...
                    <div class="type col-md-5 col-sm-6">
                        <form action="{{ url('/dashboard/pages/filter') }}" method="get">
                        <table>
                            <tr>
                                <td>
                    ...
                    <table style="width: 100%">
                        ...
                        @foreach($posts as $key => $post)
                            ...
                            <tr class="@if($key%2==0) genap @else ganjil @endif">
                                ...
                                <td>
                                    <h5>{{ $post->title }}</h5>
                                    <a href="{{ url('/dashboard/pages/'.$post->id.'/edit') }}" >Edit</a> |
                                    ...
                                    @else
                                        ...
                                        <a href="{{ url('/dashboard/pages/'.$post->id.'/change/trash') }}" style="color: red;">Trash</a> |

                                    @endif
                                    <a href="{{ url('/dashboard/pages/'.$post->id.'/detail') }}" >Detail</a>
                                </td>
            ...
            <div class="modal-dialog modal-sm" role="document">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form action="{{ url('/dashboard/pages/delete') }}" id="formDeletePost" method="post">
                            {{ method_field('DELETE') }}
                            {{ csrf_field() }}
...
@section('javascript')
    ...
    <script type="text/javascript">
        ...
        $('#move_trash').on('click', function(){
            ...
            var url = "{{ url('/dashboard/pages/move') }}";
            var posts = "{{ url('/dashboard/pages') }}";
            var arrayId = []; 
        ...
        $('#delete_trash').on('click', function(){
            ...
            var $form = $('form#action');
            var id = $form.serializeArray();
            var url = "{{ url('/dashboard/pages/delete_trash') }}";
            var posts = "{{ url('/dashboard/pages') }}";
            var arrayId = []; 
